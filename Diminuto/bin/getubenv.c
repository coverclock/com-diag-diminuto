...
/**
 * @file
 * @copyright Copyright 2008-2011 Digital Aggregates Corporation, Colorado, USA.
 * @note Licensed under the terms in LICENSE.txt.
 * @brief Prints the value of a U-Boot environmental variable from the flash.
 ...
 * getubenv<BR>
 * getubenv ipaddr<BR>
 * IPADDR="`getubenv ipaddr`"<BR>
 *
 * ABSTRACT
 *
 * Prints the value of an U-Boot environmental variable from the
 * persistent flash environment to standard output, or all variables
 * if no variable name is specified.
 *
 * Prints an error message to standard error and returns a non-zero
 * error code if the environment's checksum does not match.
 *
 ...
 * ATTRIBUTIONS
 *
 * CRC32 code is from zlib-1.1.3<BR>
 * Jean-loup Gailly and Mark Adler<BR>
 * Copyright 1995-1998 Mark Adler<BR>
 *
 * General environnment layout is from u-boot-1.1.1<BR>
 * Wolfgang Denk, DENX Software Engineering<BR>
...
static crc32_t crc32(crc32_t crc, uint8_t * buf, size_t len)
{
...
int main(int argc, char * argv[])
{
    ...
    stored = *(crc32_t *)address;
    data = (uint8_t *)address + sizeof(crc32_t);
    size = CFG_ENV_SIZE - sizeof(crc32_t);
    ...
    /*
     * The checksum matched, so we make the naive assumption that the
     * environment is formatted correctly with the proper terminators.
     */
    ...
    while (*data != '\0') {
        ...
        if (name == NULL) {
            printf("%s\n", keyword);
        } else if (strncmp(name, keyword, value - keyword - 1) == 0) {
            printf("%s\n", value);
            result = 0;
            break;
        }
