 ...
 * performance and capacity. IT IS DESTRUCTIVE TO ANY DATA ON THE STORAGE
 * DEVICE.
 *
 * It bypasses the Linux block cache by using direct synchronous I/O. That means
 * it takes a long time to run. But when it finishes successfully, you can be
 * pretty confident it has actually wiped the device from at least casual
 * inspection. The pattern it writes is generated by a polynomial initialized by
 * a random number generator seeded by the system time, so it's difficult to
 * predict. The polynomial was borrowed from the "f3" flash testing software.
 *
 * REFERENCES
 *
