...
static diminuto_list_t head = DIMINUTO_LIST_INITIALIZER;
...
static void initialize()
{
    ...
    if (first) {
        ASSERT(diminuto_list_isuninitialized(&head));
        ASSERT(diminuto_list_initif(&head) == &head);
        first = 0;
    } else {
...
int main(void)
{
...

    {
        ...
        ASSERT(head.next == DIMINUTO_LIST_NULL);
        ASSERT(head.prev == DIMINUTO_LIST_NULL);
        ASSERT(head.root == DIMINUTO_LIST_NULL);
        ASSERT(diminuto_list_isuninitialized(&head));
        initialize();
