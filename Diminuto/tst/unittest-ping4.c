 ...
 * @brief This is a unit test of the Ping feature for IPv4.
 * @author Chip Overclock <mailto:coverclock@diag.com>
 * @see Diminuto <https://github.com/coverclock/com-diag-diminuto>
 * @details
 * This is a unit test of the Ping feature for IPv4.
 *
...
int main(int argc, char * argv[])
{
...

    {
        ...
        if ((Address != (const char *)0) || (Port != (const char *)0)) {
            ASSERT(diminuto_ipc4_source(sock, from, port) >= 0);
        }
...

    {
        ...
        for (ss = 0; ss < 10; ++ss) {
            ...
            CHECKPOINT("from=\"%s\" size=%zu type=0x%x code=0x%x id=0x%x seq=%u ttl=%u elapsed=%lluticks\n", diminuto_ipc4_address2string(from, buffer, sizeof(buffer)), size, type, code, id, seq, ttl, (long long unsigned int)elapsed);
            ASSERT(from != 0);
            EXPECT(from == to);
            ASSERT(type != ~0);
            ASSERT(code != ~0);
