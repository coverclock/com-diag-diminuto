...
VALUE=0
linetool ${DEBUG} -D ${HARDWARE_TEST_FIXTURE_DEVICE} -p ${HARDWARE_TEST_FIXTURE_PIN_BUT_HIGH} -i -H -x -1 -b ${HARDWARE_TEST_FIXTURE_DEBOUNCE_CUE_USEC} | while read PIN27; do
	PIN22=$(linetool ${DEBUG} -D ${HARDWARE_TEST_FIXTURE_DEVICE} -p ${HARDWARE_TEST_FIXTURE_PIN_BUT_LOW} -i -L -x -r)
	if [[ ${PIN22} -ne 0 ]]; then
		break
...
VALUE=0
linetool ${DEBUG} -D ${HARDWARE_TEST_FIXTURE_DEVICE} -p ${HARDWARE_TEST_FIXTURE_PIN_BUT_HIGH} -i -H -R -M ${HARDWARE_TEST_FIXTURE_DEBOUNCE_DEV_USEC} -x -1 -m ${HARDWARE_TEST_FIXTURE_DEBOUNCE_MUX_USEC} | while read PIN27; do
echo "PIN27 ${PIN27}" 1>&2
	PIN22=$(linetool ${DEBUG} -D ${HARDWARE_TEST_FIXTURE_DEVICE} -p ${HARDWARE_TEST_FIXTURE_PIN_BUT_LOW} -i -L -x -r)
echo "PIN22 ${PIN22}" 1>&2
	if [[ ${PIN22} -ne 0 ]]; then
		break
	elif [[ ${PIN27} -ne 0 ]]; then
		VALUE=$((${VALUE} + 1))
