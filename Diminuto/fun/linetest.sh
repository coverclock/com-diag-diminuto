...
VALUE=0
linetool ${DEBUG} -D ${HARDWARE_TEST_FIXTURE_DEVICE} -p ${HARDWARE_TEST_FIXTURE_PIN_BUT_HIGH} -i -H -R -1 -U -M ${HARDWARE_TEST_FIXTURE_DEBOUNCE_DEV_USEC} -x -m ${HARDWARE_TEST_FIXTURE_DEBOUNCE_MUX_USEC} | while read PIN27; do
	PIN22=$(linetool ${DEBUG} -D ${HARDWARE_TEST_FIXTURE_DEVICE} -p ${HARDWARE_TEST_FIXTURE_PIN_BUT_LOW} -i -L -x -r)
	if [[ ${PIN22} -ne 0 ]]; then
		break
	elif [[ ${PIN27} -ne 0 ]]; then
