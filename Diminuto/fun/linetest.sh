...
VALUE=0
linetool ${DEBUG} -D ${HARDWARE_TEST_FIXTURE_DEVICE} -p ${HARDWARE_TEST_FIXTURE_PIN_BUT_HIGH} -i -H -x -1 -b ${HARDWARE_TEST_FIXTURE_DEBOUNCE_LIB_USEC} | while read PIN27; do
	PIN22=$(linetool ${DEBUG} -D ${HARDWARE_TEST_FIXTURE_DEVICE} -p ${HARDWARE_TEST_FIXTURE_PIN_BUT_LOW} -i -L -x -r)
	if [[ ${PIN22} -ne 0 ]]; then
		break
...
VALUE=0
linetool ${DEBUG} -D ${HARDWARE_TEST_FIXTURE_DEVICE} -p ${HARDWARE_TEST_FIXTURE_PIN_BUT_HIGH} -i -H -R -M ${HARDWARE_TEST_FIXTURE_DEBOUNCE_DEV_USEC} -x -1 -m ${HARDWARE_TEST_FIXTURE_TIMEOUT_MUX_USEC} | while read PIN27; do
	PIN22=$(linetool ${DEBUG} -D ${HARDWARE_TEST_FIXTURE_DEVICE} -p ${HARDWARE_TEST_FIXTURE_PIN_BUT_LOW} -i -L -x -r)
	if [[ ${PIN22} -ne 0 ]]; then
		break
	elif [[ ${PIN27} -ne 0 ]]; then
